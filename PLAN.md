...
## Done
- 2026-06-06 — **Pre-rollout hardening** (while waiting for device access): verified the legacy-history migration end-to-end in the simulator (seeded old `feedingTimes`, confirmed it imported + pushed to the backend); added `server/smoke.sh` (12 API assertions, throwaway family, self-cleaning — run after every deploy); replaced the app icon with a 🤱-on-warm-gradient design (regenerated all AppIcon slots); removed the `-startTab` test hook so the app entry is production-clean; added `RUNBOOK.md` (two-phone install steps + acceptance checklist).
